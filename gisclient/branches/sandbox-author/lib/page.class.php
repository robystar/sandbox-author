	...
	*/
	...
	class page{
		...
		function writeMenuNav(){
			...
			$tmp=parse_ini_file(ROOT_PATH.$rel_dir.'menu.ini',true);
			$this->navTreeValues=$tmp;
			//print_array($this->navTreeValues);
			$lbl="<a class=\"link_label home\" href=\"#\" title=\"Home\" onclick=\"javascript:navigate([],[])\">&nbsp;</a>";
			...
			if ($n_elem>0){
				...
				foreach($this->parametri as $key=>$value){
					...
					if ((is_numeric($value) && $value>0) || (!is_numeric($value) && strlen($value)>0))
						$lbl.="<a class=\"link_label next\" href=\"#\" title=\"".$key."\" onclick=\"javascript:navigate(['".@implode("','",$lvl)."'],['".@implode("','",$val)."'])\">$navTreeTitle</a>";
					else
						$lbl.="<a class=\"link_label next\" title=\"".$key." href=\"#\"> $navTreeTitle</a>";
				}
			}
			echo "
		...
		//Metodo che scrive il Form in modalitï¿½ List  Elenco dei Child
		
		 function writeListForm($tab,$el,&$prm){
			//print_debug($this,null,'PAGINA');
			$tb=new Tabella_h($tab["config_file"].".ini","list");
			$tb->savedata=(!empty($tab["save_data"]))?($tab["save_data"]):(null);
		...
		//Metodo che scrive il Form in modalitï¿½ View
		
		private function writeViewForm($tab,$el,&$prm){
			$frm = '';
			switch ($tab["tab_type"]){
				case 1:	// MODALITA' VIEW STANDARD (TABELLA V)
					$tb=new Tabella_v($tab["config_file"].".ini","view");
		...
		private function writeEditForm($tab,$el,&$prm){
			switch ($tab["tab_type"]){
				case 110:
					$prm["livello"]=$tab["level"];
					$prm["config_file"]=$tab["config_file"].".ini";
				...
				case 100: //Tabella H per elencare tutti i valori possibili e quelli selezionati
					...
					$tb=new Tabella_h($tab["config_file"].".ini","edit");
					for($j=0;$j<count($tb->function_param);$j++) $tb->function_param[$j]=$this->parametri[$tb->function_param[$j]];
					echo "<form name=\"frm_data\" id=\"frm_data\" enctype=\"multipart/form-data\" action=\"".$_SERVER["PHP_SELF"]."\" method=\"POST\">";
					$tb->set_titolo($tb->FileTitle,"",$prm);
					$tb->tag=$tab["level"];
				...
				case 0:		//SERVE PER ELENCARE I VALORI IN FUNZIONE DEL PARENT (TABELLA H)
					...
					$tb=new Tabella_h($tab["config_file"].".ini",$mode);
				...
				case 50:
					...
					$tb=new Tabella_v($tab["config_file"].".ini","edit");
					for($j=0;$j<count($tb->function_param);$j++) $tb->function_param[$j]=$this->parametri[$tb->function_param[$j]];	
					$j=0;
					$e=array_pop($this->levKey);
					$j=0;
		...
		//Metodo che scrive il Form in modalitï¿½ NEW
		private function writeNewForm($tab,$el,&$prm){
			...
			switch ($tab["tab_type"]){
				...
				case 0:
					...
					$prm["parametri[][".$tab["level"]."]"]="";
					if (is_array($el) && $el["value"]) $filter=$tab["parent_name"]."_id = ".$this->db->quote($el["value"]);
					$tb=new Tabella_h($tab["config_file"].".ini","new");
					for($j=0;$j<count($tb->function_param);$j++) $tb->function_param[$j]=$this->parametri[$tb->function_param[$j]];
					$tb->set_titolo($tb->FileTitle,"",$prm);
					$tb->tag=$tab["level"];
					$tb->get_titolo();
				...
				case 2:
					...
					if($this->action=='wizard wms'){
						$prm["config_file"]="layergroup_wms.ini";
		...
		function writePage($err=Array()){
			...
			if(!empty($this->tableList)){
				...
				$table=new Tabella_v($this->tableList[0]["config_file"].".ini");
...
<div id=\"i18n_inline\">
	<div class=\"tableHeader ui-widget ui-widget-header ui-corner-top\">
		<b>".GCAuthor::t('translation')."</b>
		<button id='i18n'>".GCAuthor::t('button_edit')."</button>
	</div>
	<table id=\"i18n_view_table\" class=\"stiletabella\" style=\"margin-top:10px;margin-bottom:10px;\">
	</table>
</div>
<div id=\"i18n_dialog\" style=\"display:none;\">
	<table id=\"i18n_edit_table\" class=\"stiletabella\" style=\"margin-top:10px;\">
	</table>
	<input type=\"button\"  name=\"submit\" value=\"".GCAuthor::t('button_save')."\">
</div>
";
							}
							else{
		...
		function writeForm($conf,$mode){
			switch($mode){
				case "new":
					$table=new Tabella_v("$conf.ini");
					break;
				case "edit":
					$table=new Tabella_v("$conf.ini");
					break;
